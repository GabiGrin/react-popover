...
import { getWindow, isServer } from "./platform"
import { noop } from "./utils"

const requestAnimationFrame = isServer()
  ? noop
  : getWindow().requestAnimationFrame ||
    getWindow().mozRequestAnimationFrame ||
    getWindow().webkitRequestAnimationFrame ||
    ((fn) => { getWindow().setTimeout(fn, 20) })

const cancelAnimationFrame = isServer()
  ? noop
  : getWindow().cancelAnimationFrame ||
    getWindow().mozCancelAnimationFrame ||
    getWindow().webkitCancelAnimationFrame ||
    getWindow().clearTimeout
...
const on = (el, fn) => {
  ...
  if (el === getWindow()) {
    getWindow().addEventListener("resize", fn)
    return
  }
...
const off = (el, fn) => {
  if (el === getWindow()) {
    getWindow().removeEventListener("resize", fn)
    return
  }
  const detector = el[namespace]
  if (!detector) return
