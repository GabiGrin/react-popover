...
import { isServer, getWindow, getDocument } from "./platform"
...
const Popover = createClass({
  ...
  trackPopover () {
    ...
    /* Note: frame is hardcoded to getWindow now but we think it will
    ...
    getDocument.addEventListener("mousedown", this.checkForOuterAction)
    getDocument.addEventListener("touchstart", this.checkForOuterAction)
  ...
  untrackPopover () {
    ...
    getDocument.removeEventListener("mousedown", this.checkForOuterAction)
    getDocument.removeEventListener("touchstart", this.checkForOuterAction)
  },
  onTargetResize () {
